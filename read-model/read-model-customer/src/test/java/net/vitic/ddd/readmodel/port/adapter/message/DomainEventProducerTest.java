...
import static net.vitic.ddd.readmodel.port.adapter.message.DomainEventProducer.MSG_HEADER_EVENT_ID;
import static net.vitic.ddd.readmodel.port.adapter.message.DomainEventProducer.MSG_HEADER_EVENT_TYPE;
    ...
    void should_do_something() {
        ...
        verify(messageChannel).send(
            argThat((Message message)->
                Objects.equals(message.getHeaders().get(MSG_HEADER_EVENT_ID), CUSTOMER_ID) &&
                Objects.equals(message.getHeaders().get(MSG_HEADER_EVENT_TYPE),CustomerCreated.class.getName() + ":0") &&
